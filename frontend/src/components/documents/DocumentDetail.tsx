...
function isRecord(value: DocumentDataValue | undefined): value is Record<string, DocumentDataValue | undefined> {
...
function formatPrimitiveValue(key: string, value: string | number | boolean | null): string {
...
function renderValue(name: string, value: DocumentDataValue | undefined): JSX.Element {
      ...
      return (
        <div className="flex flex-wrap gap-2 mt-1">
          {value.map((item, index) => (
            ...
            >
              {formatPrimitiveValue(name, item as string | number | boolean | null)}
            </span>
          ))}
        </div>
      )
  ...
  return <EntryGrid entries={Object.entries(value).map(([key, item]) => ({ label: key, value: item }))} />
}
  ...
  return (
    <div className="space-y-2">
      {entities.map((entity, index) => {
        ...
        const extraEntries = Object.entries(rest).map(([key, value]) => ({ label: key, value }))

        return (
          <div key={`${label ?? 'entity'}-${index}`} className="rounded-lg border border-border bg-card px-4 py-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-sm font-medium text-foreground break-words">
                  {typeof text === 'string' && text.trim().length > 0 ? text : 'Entité sans texte'}
                </p>
              </div>
              <Badge variant="outline" className="shrink-0">
                {typeof label === 'string' && label.trim().length > 0 ? label : 'Sans label'}
              </Badge>
            </div>
...
export function DocumentDetail({ document }: DocumentDetailProps) {
  ...
  const fieldsEntries = Object.entries(document.fields ?? {}).map(([label, value]) => ({ label, value }))
  const entities = Array.isArray(document.donneesExtraites?.entities)
    ? document.donneesExtraites.entities.filter((entity): entity is ExtractedEntity => isRecord(entity))
    : []
  const extractedText = typeof document.donneesExtraites?.text === 'string'
    ? document.donneesExtraites.text.trim()
    : ''

  const summaryEntries: DetailEntry[] = [
    { label: 'nomFichierDOrigine', value: document.nomFichierDOrigine ?? document.name },
    { label: 'filename', value: document.donneesExtraites?.filename },
    { label: 'dateTraitement', value: document.dateTraitement ?? document.date },
    { label: 'type', value: typeLabels[document.type] },
    { label: 'status', value: document.status },
    { label: 'entitiesCount', value: entities.length > 0 ? entities.length : undefined },
  ]
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {fieldsEntries.length === 0 && anomalies.length === 0 && entities.length === 0 && extractedText.length === 0 && (
        <div className="rounded-lg border border-dashed border-border px-4 py-6 text-center">
          <CalendarClock size={18} className="mx-auto text-muted-foreground" />
          <p className="mt-2 text-sm text-muted-foreground">Aucune donnée extraite à afficher.</p>
        </div>
      )}
    </div>
  )
