...
function toDocumentDataValue(value: unknown): DocumentDataValue | undefined {
  ...
  if (Array.isArray(value)) {
    return value
      ...
      .filter((item): item is Exclude<DocumentDataValue, undefined> => item !== undefined)
  }
...
function normalizeExtractedData(value: unknown): ExtractedData | undefined {
  ...
  if (!normalized || Array.isArray(normalized) || typeof normalized !== 'object') return undefined

  return normalized as ExtractedData
}

function inferDocumentType(raw: RawDocument, extracted?: ExtractedData): Document['type'] {
  const haystack = [
    raw.type,
  ...
  if (haystack.includes('urssaf') || haystack.includes('attestation')) return 'attestation_urssaf'
...
function collectFields(raw: RawDocument, extracted?: ExtractedData): Document['fields'] {
  ...
  const extractedEntries = Object.entries(extracted).filter(([key, value]) => (
